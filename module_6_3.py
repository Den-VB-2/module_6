...
Замечали, что некоторые животные в нашем мире обладают странными и, порой, несовместимыми друг
с другом свойствами? Например, утконос... Вроде есть клюв, но не птица. Вроде милый, а есть шипы
на задних лапах. А ещё он откладывает яйца... Опустим факт о том, что они потеют молоком
...
И методами:
move(self, dx, dy, dz), который должен менять соответствующие кооординаты в _cords на dx, dy и dz
в том же порядке, где множетелем будет являтся speed. Если при попытке изменения координаты z
в _cords значение будет меньше 0, то выводить сообщение "It's too deep, i can't dive :
...
При определении порядка наследования обратите внимание на то, что утконос атакует
"Be careful, i'm attacking you 0_0"
"""

# Переведено и озвучено студией "полный Пэ"
from random import randint

class Animal:
    live = "Утконос живой и здоровый"
    sound = None
    _DEGREE_OF_DANGER = 0
    _cords = [0, 0, 0]
    ...
    def move(self, dx, dy, dz):
        if self._cords[2] + dz * self.speed < 0:
            ...
            self._cords = [int(self._cords[0] + dx * self.speed),
                           int(self._cords[1] + dy * self.speed),
                           int(self._cords[2] + dz * self.speed)]
    ...
    def attack(self):
        if self._DEGREE_OF_DANGER < 5:
            ...
            print("Будь осторожен, я тебя атакую!!!")
...
class Bird(Animal):
    ...
    def lay_eggs(self):
        ...
        if num_of_eggs > 1:
            print(f"Вот, есть {num_of_eggs} яйца!")
        else:
            print(f"Вот, есть всего одно яйцo")
...
class Duckbill(PoisonousAnimal, Bird, AquaticAnimal):
    sound = "Клац! Клац! Клац! Ща клюну!!!"  # утконос
